...
public class BeanUtil extends BeanUtils {
			...
			if( isExistBeanFiled( source , sourceProperties[loop] , source.getClass() ) 
				...
				try{
					...
					if( value == null ){
						...
						BeanUtils.setProperty(desc, StringLang.toPropertyWithHump(descProperties[loop]), value);
					}
				}catch(Exception e){
					//Skip ...
				}
	...
	private static boolean isExistBeanFiled( Object bean , String fieldName , Class<?> clazz ){
		...
		for( Field field : fields ){
			if( field.getName().equals( fieldName )  || field.getName().equals( StringLang.toPropertyWithHump(fieldName) )){
				return true ;
			}
		}
		return false ;
...
//			properties[ i ] = StringLang.toPropertyWithHump(fields[ i ].getName()) ;
		}
	...
	public static String TrimString(String strs){
	...
	public static String StringToSqlStr(String strs){
		String[] strings = strs.split(",");
	...
	public static void convertParam( Object bean ){
		if( bean == null ) return ;
		Field [] fields = bean.getClass().getDeclaredFields() ;

		for (Field field : fields) {
			if (Reflection.getFieldValue( bean , field.getName()).equals("")){
					Reflection.setFieldValue(bean, field.getName(),null);
			}
		}

	}

	public static <T> T checkRequestParam(T source)  {
		Field[] fields = source.getClass().getDeclaredFields();
		for (Field field : fields) {
	...
	public static <T> T checkPrintParam(T source)  {
		Field[] fields = source.getClass().getDeclaredFields();
		for (Field field : fields) {
				field.setAccessible(true);
	 ...
	 * @return
	 */
	public static String convertTagValue(List<TagValueVo> tagValueVos) {
	 ...
	 * @return
	 */
	public static String convertTagListValue(List<TagValueVo> tagValueVos) {
		if (tagValueVos.isEmpty() || tagValueVos.size() <= 0){
			return "";
		}
	...
	public static String convertTagListTagValue(List<TagValueVo> tagValueVos) {
		if (tagValueVos.isEmpty() || tagValueVos.size() <= 0){
			return "";
		}
		...
		return tagValueVos.get(0).getTagName();
	}
}
