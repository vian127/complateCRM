...
public class CustomerService extends AbstractBaseService {
   ...
   */
    ...
    if (paramVo.getId() != null && !paramVo.getId().equals("")){
      ...
      customer.setUpdateroleid(paramVo.getUserRoleId());//更新人
      ...
      customer.setLastupdatetime(TimeUtil.currentTime());//更新时间
  ...
  public void updateCustomerTag(CustomerParamVo paramVo) {
    ...
    for (TagParamVo tagParamVo : paramVo.getTags()) {
      ...
      tagCustomer.setCreateroleid(paramVo.getUserRoleId());//创建人id
      tagCustomer.setCreater(paramVo.getUserRoleName());//创建人
