...
public class PoiPrintUtils {
    ...
    //构造方法，传入要导出的数据
    public PoiPrintUtils(String title, String[] rowName, List<Object[]>  dataList){
      ...
      try{
        ...
        if(workbook !=null){
          try
          {
            ...
            String fileName = "云咖啡用户统计_"+TimeUtil.currentTime("yyyy-MM-dd")+".xls";// 文件名
            response.reset();
...
//            FileOutputStream out = new FileOutputStream("D:/data/" + 1+".xls");
            //            workbook.write(out);
            //            out.close();
          }
          catch (IOException e)
