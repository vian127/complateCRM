    ...
    $(function () {
        ...
        var vm = new Vue({
            ...
            mounted:function(){
                ...
                window.global_filter_fn={                                                            //重置
                    resetData:function (){
                        ...
                        vm.cutomer_tags.feedInvoice['feedbtagval0000000006'].forEach(function(item){
                            item.is_checked=false;
                        })
                        vm.cutomer_tags.feedInvoice['feedbtagval0000000007'].forEach(function(item){
                            item.is_checked=false;
                        })
                        vm.def.tags={};
            ...
            methods:{
                ...
                feedInvoiceFun:function(e,id){                                          //获取发票详细信息
                    var tag=e.target;
                    if(tag.checked==true){
                        this.collect.feedbtagval.push(id)
                    }else{
                        var index=this.collect.feedbtagval.indexOf(id);
                        if(index!=-1){
                            this.collect.feedbtagval.splice(index,1)
                        }
                    }
                },
                feedInvoiceRest:function(){                                            //切换清空发票详细信息
                    this.collect.feedbtagval=[];
                    if(this.collect.feedInvoice=='feedbtagval0000000006' || this.collect.feedInvoice=='feedbtagval0000000007'){
                        this.resetFeedInvoice(this.collect.feedInvoice);
                    }
                },
                resetFeedInvoice:function(val){                                             
                    this.cutomer_tags.feedInvoice[val].forEach(function(item){
                        item.is_checked=false;
                    })
                },
                bindFunc:function(){
